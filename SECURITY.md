...
- Dependencies are locked and audited in CI (fail on high/critical advisories on every push); the full production dependency tree has been reviewed for license compatibility (permissive or compatible weak-copyleft only — no GPL/AGPL/LGPL). Releases will be published to npm with provenance starting at v0.1.0.

## Supported versions

Pre-release (0.1.0 not yet published) — no supported-versions table yet. Once published:
